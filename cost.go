...
// CostReport is everything line 2's money segments need.
...
// buildCostReport brings the incremental scan up to date and aggregates it.
func buildCostReport(in *StatusLineInput, now time.Time) *CostReport {
	rep := &CostReport{}
	...
	statePath := costStatePath()
	st := loadCostState(statePath)
	st.scan(projectsDir(), now)
	st.save(statePath)
...
func aggregate(st *costState, rep *CostReport, now time.Time) {
...
// costSegments renders the 💰 and 🔥 segments.
//
...
func costSegments(rep *CostReport, dropBlock bool) []string {
	if rep == nil {
		return nil
	}
