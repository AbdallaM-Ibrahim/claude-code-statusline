...
// state is the incremental scan's memory between renders.
//
...
func (st *state) save(path string) {
...
func (st *state) scan(root string, now time.Time) {
...
func (st *state) consume(path string, offset, horizonHour int64) int64 {
	f, err := os.Open(path)
	if err != nil {
		return offset
	}
